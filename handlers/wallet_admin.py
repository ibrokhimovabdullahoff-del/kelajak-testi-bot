...
class WalletAdjustState(StatesGroup):
    user_id = State(); amount = State(); note = State()
...
async def _apply_adjustment(message: Message, user_id: int, amount: int, note: str) -> None:
    ...
    if not ok:
        ...
        return
    await message.answer(f"✅ User <code>{user_id}</code>: {amount:+,} so‘m\nYangi balans: <b>{money(await wallet.balance(user_id))} so‘m</b>")

@router.message(Command("walletadjust", "adjustwallet"))
async def wallet_adjust_command(message: Message, state: FSMContext) -> None:
    ...
    if len(parts) >= 3 and parts[1].isdigit() and parts[2].split(maxsplit=1)[0].lstrip("-").isdigit():
        user_id = int(parts[1]); value_parts = parts[2].split(maxsplit=1); amount = int(value_parts[0]); note = value_parts[1] if len(value_parts) > 1 else "Admin adjustment"
        await state.clear(); await _apply_adjustment(message, user_id, amount, note); return
    await state.clear(); await state.set_state(WalletAdjustState.user_id)
...
async def wallet_adjust_cancel(message: Message, state: FSMContext) -> None:
    await state.clear(); await message.answer("❌ Bekor qilindi.")
@router.message(WalletAdjustState.user_id)
async def wallet_adjust_user_id(message: Message, state: FSMContext) -> None:
    raw=(message.text or "").strip()
    if not raw.isdigit(): return await message.answer("❌ User ID noto‘g‘ri. Faqat raqam yuboring.")
    await state.update_data(user_id=int(raw)); await state.set_state(WalletAdjustState.amount); await message.answer("💰 Summani yuboring.\n\nQo‘shish: <code>50000</code>\nAyirish: <code>-5000</code>\n\nBekor qilish: /bekor")
...
async def wallet_adjust_amount(message: Message, state: FSMContext) -> None:
    ...
    if not raw.lstrip("-").isdigit(): return await message.answer("❌ Summa noto‘g‘ri. Masalan: <code>50000</code> yoki <code>-5000</code>")
    amount=int(raw)
    if amount==0:return await message.answer("❌ Summa 0 bo‘lishi mumkin emas.")
    await state.update_data(amount=amount); await state.set_state(WalletAdjustState.note); await message.answer("📝 Izoh yuboring yoki <code>/skip</code> deb yozing.\n\nMasalan: <i>Admin bonus</i>")
...
async def wallet_adjust_skip_note(message: Message, state: FSMContext) -> None:
    data=await state.get_data(); await state.clear(); await _apply_adjustment(message,int(data["user_id"]),int(data["amount"]),"Admin adjustment")
@router.message(WalletAdjustState.note)
async def wallet_adjust_note(message: Message, state: FSMContext) -> None:
    data=await state.get_data(); note=(message.text or "").strip()[:500]
    if not note:return await message.answer("❌ Izoh bo‘sh bo‘lmasin yoki /skip yuboring.")
    await state.clear(); await _apply_adjustment(message,int(data["user_id"]),int(data["amount"]),note)
