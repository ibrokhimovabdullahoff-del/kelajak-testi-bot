...
ITEMS = [
    ...
    Item("boshqaruv", L("Farzandingiz boshlagan ishini chalg‘imasdan oxiriga yetkazadimi?",
         ...
         U("oxiriga yetkazadi"), U("oxiriga yetkazmaydi")),
    Item("boshqaruv", L("O‘yinda yoki suhbatda navbatini kuta oladimi?",
                        "Умеет ли он дождаться своей очереди в игре или разговоре?"),
    ...
    Item("boshqaruv", L("Xohlagani darhol bo‘lmasa, jahli chiqadimi?",
         ...
         U("jahli chiqadi"), U("jahli chiqmaydi"), reverse=True),
    # Qiziquvchanlik
    Item("qiziqish", L("Atrofidagi narsalar haqida ko‘p savol beradimi?",
                       "Задаёт ли он много вопросов о том, что его окружает?"),
         U("savol beradi"), U("savol bermaydi")),
    Item("qiziqish", L("Kitob o‘qishni (yoki unga o‘qib berishingizni) yoqtiradimi?",
                       "Любит ли он читать книги (или когда ему читают)?"),
         U("yoqtiradi"), U("yoqtirmaydi"), kind="deg"),
    ...
    Item("tafakkur", L("Farzandingiz harakat qilganida, natijasidan qat’i nazar, mehnati uchun maqtaysizmi?",
                       "Хвалите ли вы ребёнка за старание, независимо от результата?"),
         U("maqtayman"), U("maqtamayman")),
    Item("tafakkur", L("Xato qilsa, qaytadan urinib ko‘radimi?",
                       "Пробует ли он снова, если ошибся?"),
         U("qayta urinib ko‘radi"), U("urinib ko‘rmaydi")),
    ...
    Item("ijtimoiy", L("Boshqa odam xafa bo‘lsa, buni sezib, hamdardlik qiladimi?",
                       "Замечает ли он, когда другому плохо, и сочувствует ли?"),
         U("hamdardlik qiladi"), U("hamdardlik qilmaydi")),
    ...
    # Mehnatsevarlik
    Item("mehnat", L("Uyda uning doimiy vazifasi bormi? Masalan, idish yig‘ish yoki xonasini tartibga keltirish.",
                     "Есть ли у него постоянные обязанности дома? Например, убрать посуду или свою комнату."),
         U("bor"), U("yo‘q"), kind="yesno"),
    Item("mehnat", L("Vazifasini eslatmasangiz ham o‘zi bajaradimi?",
                     "Выполняет ли он свои обязанности без напоминаний?"),
         U("o‘zi bajaradi"), U("o‘zi bajarmaydi")),
    Item("mehnat", L("Boshqalarga yordam berishni yoqtiradimi?",
                     "Любит ли он помогать другим?"),
         U("yoqtiradi"), U("yoqtirmaydi"), kind="deg"),
    # Uydagi muhit
    Item("oila", L("Farzandingiz bilan telefonsiz, faqat unga ajratilgan vaqt o‘tkazasizmi?",
                   "Проводите ли вы время с ребёнком без телефона, только для него?"),
         U("vaqt o‘tkazaman"), U("o‘tkazmayman")),
    Item("oila", L("Uni tinglab, fikrini so‘raysizmi?",
                   "Слушаете ли вы его и спрашиваете его мнение?"),
         U("fikrini so‘rayman"), U("so‘ramayman")),
    Item("oila", L("Oilangizdagi qoidalar kayfiyatga qarab o‘zgarmay, bir xil qo‘llanadimi?",
                   "Применяются ли правила в семье одинаково, независимо от настроения?"),
         U("bir xil qo‘llanadi"), U("bir xil qo‘llanmaydi")),
    # Uyqu va harakat. Ota-ona "yetarli uxlaydimi?" ni baholay olmaydi —
    # ertalabki holatni esa har kuni ko'radi.
    Item("sogliq", L("Ertalab o‘zi osongina, charchamay uyg‘onadimi?",
                     "Просыпается ли он утром сам, легко и бодро?"),
         U("osongina uyg‘onadi"), U("osongina uyg‘onmaydi")),
    Item("sogliq", L("Kuniga telefon, planshet yoki televizor oldida qancha vaqt o‘tkazadi (darsdan tashqari)?",
                     "Сколько времени в день он проводит у телефона, планшета или телевизора (не считая учёбы)?"),
         kind="custom", reverse=True, options={
             "uz": ["1 soatdan kam", "1–2 soat", "2–3 soat", "3–5 soat", "5 soatdan ko‘p"],
             "ru": ["Меньше 1 часа", "1–2 часа", "2–3 часа", "3–5 часов", "Больше 5 часов"],
         }),
    Item("sogliq", L("Haftada necha marta sport yoki faol o‘yin bilan shug‘ullanadi?",
                     "Сколько раз в неделю он занимается спортом или активными играми?"),
         kind="custom", options={
             "uz": ["Umuman shug‘ullanmaydi", "Haftada 1 marta", "Haftada 2 marta",
                    "Haftada 3–4 marta", "Deyarli har kuni"],
             "ru": ["Совсем не занимается", "1 раз в неделю", "2 раза в неделю",
                    "3–4 раза в неделю", "Почти каждый день"],
         }),
    # Hissiy holat
    Item("hissiyot", L("Xafa bo‘lsa, buni sizga aytadimi?",
                       "Говорит ли он вам, когда расстроен?"),
         U("sizga aytadi"), U("aytmaydi")),
    Item("hissiyot", L("Yangi joy yoki yangi odamdan qattiq qo‘rqadimi?",
                       "Сильно ли он боится новых мест и новых людей?"),
         U("qattiq qo‘rqadi"), U("qo‘rqmaydi"), reverse=True),
    Item("hissiyot", L("Kayfiyati sababsiz, keskin o‘zgarib turadimi?",
                       "Бывает ли, что его настроение резко меняется без причины?"),
         U("keskin o‘zgaradi"), U("o‘zgarmaydi"), reverse=True),
]
